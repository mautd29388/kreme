...
add_filter('woocommerce_product_get_rating_html', 'kreme_woocommerce_product_get_rating_html', 20, 2);
function kreme_woocommerce_product_get_rating_html( $rating_html, $rating ) {
	...
	return '<div class="star-rating" title="' . __( 'No ratings', 'kreme' ) . '"></div>';
}

/**
 * Define image sizes
 */
add_action( 'after_switch_theme', 'kreme_woocommerce_image_dimensions', 1 );
function kreme_woocommerce_image_dimensions() {
	global $pagenow;
...
function kreme_woocommerce_product_thumbnails_columns($columns){
...
add_action( 'woocommerce_after_shop_loop', 'kreme_pagination', 20 );

add_filter('woocommerce_breadcrumb_defaults', 'kreme_woocommerce_breadcrumb_defaults', 10, 2);
function kreme_woocommerce_breadcrumb_defaults($breadcrumb){
...
add_filter('woocommerce_sale_flash', 'kreme_woocommerce_sale_flash', 10, 2);
function kreme_woocommerce_sale_flash() {
	echo '<span class="onsale">' . __( 'Sale', 'kreme' ) . '</span>';
}

add_action( 'woocommerce_before_shop_loop', 'kreme_woocommerce_set_layout', 9 );
function kreme_woocommerce_set_layout() { 
 ...
 * Custom Link Filter
 * */
add_filter('woocommerce_layered_nav_link', 'kreme_woocommerce_layered_nav_link');
function kreme_woocommerce_layered_nav_link($link) {
...
add_filter('add_to_cart_fragments', 'kreme_woocommerce_header_add_to_cart_fragment');
function kreme_woocommerce_header_add_to_cart_fragment( $fragments ) {
...
add_filter('get_product_search_form', 'kreme_get_product_search_form');
function kreme_get_product_search_form($form) {
ob_start();
?>
