...
class Kreme_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'kreme_get_breadcrumb', $this->crumbs, $this );
	}
	...
	public function generate() {
		...
		if ( ( ! is_front_page() && ! ( is_post_type_archive() && function_exists('wc_get_page_id') && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	private function prepend_shop_page() {
		$permalinks   = get_option( 'kreme_permalinks' );
	...
	private function add_crumbs_404() {
		$this->add_crumb( __( 'Error 404', 'kreme' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'product' === get_post_type( $post ) ) {
			...
			if ( $terms = wc_get_product_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				$main_term = apply_filters( 'kreme_breadcrumb_main_term', $terms[0], $terms );
	...
	private function add_crumbs_product_tag() {
		...
		$this->prepend_shop_page();
		$this->add_crumb( sprintf( __( 'Products tagged &ldquo;%s&rdquo;', 'kreme' ), $current_term->name ) );
	}
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( __( 'Posts tagged &ldquo;%s&rdquo;', 'kreme' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( __( 'Author: %s', 'kreme' ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( __( 'Search results for &ldquo;%s&rdquo;', 'kreme' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( __( 'Page %d', 'kreme' ), get_query_var( 'paged' ) ) );
		}
	}
	...
	public static function instance($args) {
		...
		$args = wp_parse_args( $args, apply_filters( 'kreme_breadcrumb_defaults', array(
				'delimiter'   => '&nbsp;&#47;&nbsp;',
				'wrap_before' => '<div class="page-breadcrumb" ' . ( is_single() ? 'itemprop="breadcrumb"' : '' ) . '>',
				'wrap_after'  => '</div>',
				'before'      => '',
				'after'       => '',
				'home'        => _x( 'Home', 'breadcrumb', 'kreme' )
		) ) );
		
		$breadcrumbs = new self();
		
		if ( ! empty( $args['home'] ) ) {
			$breadcrumbs->add_crumb( $args['home'], apply_filters( 'kreme_breadcrumb_home_url', home_url() ) );
		}
